...
def building_kernel(target, data, platform):
    if "containers" in data:
        ...
        for con in data["containers"]:
            ...
            target.write("\tv++ -l $(VPP_FLAGS) ")
            target.write(
                "$(VPP_LDFLAGS) -t $(TARGET) --platform $(PLATFORM) --temp_dir "
            )
            target.write("$(TEMP_DIR)")
...
def building_kernel_rtl(target, data):
    ...
    if "containers" in data:
        for con in data["containers"]:
            ...
            target.write("\n")
            target.write("\tmkdir -p $(BUILD_DIR)\n")
            target.write("\tv++ -l $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) ")
            target.write("$(VPP_LDFLAGS) --temp_dir ")
            target.write("$(TEMP_DIR)")
...
def generate_makefile(desc_file, path, platform):
    ...
    if "match_makefile" in data and data["match_makefile"] == "false":
        ...
        target = open(os.path.join(path, "makefile_us_alveo.mk"), "w")
