...
def mk_sdcard(target, data):
    ...
    if "containers" in data:
        for con in data["containers"]:
            target.write(
                "\tv++ -p $(VPP_FLAGS) $(LINK_OUTPUT) -t $(TARGET) --platform $(PLATFORM) -o $(BUILD_DIR)/"
            )
    ...
    if "containers" in data:
        for con in data["containers"]:
            target.write(
                "\tv++ -p $(VPP_PFLAGS) $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) "
            )
            target.write(
                "--package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE)"
            )
...
def generate_makefile(desc_file, path, platform):
    ...
    if "match_makefile" in data and data["match_makefile"] == "false":
        ...
        target = open(os.path.join(path, "makefile_versal_ps.mk"), "w")
